...
from lauescript.cryst.tables import halogens
...
class AtomInterface(object):
    ...
    def update_adp_frac(self, value):
        self.adp['cart_meas'] = frac2cart_ADP(value, self.molecule.get_cell())
        self.adp['frac_meas'] = value

    def update_adp_cart(self, value):
        self.adp['frac_meas'] = cart2frac_ADP(value, self.molecule.get_cell())
        self.adp['cart_meas'] = value
    ...
    def update_cart(self, value):
        self.frac = cart2frac(value, self.molecule.get_cell())
        self.cart = value

    def set_frac(self, value):
        self.frac = value

    def update_frac(self, value):
        self.cart = frac2cart(value, self.molecule.get_cell())
        self.frac = array(value)
...
class ATOM(AtomInterface):
    ...
    def update_H_ADP(self):
        try:
            ...
            exit('error in ATOM.update_H_ADP')
            ...
            if not any(self.element == i for i in neighbors):
                if len(neighbors) > 1:
                    if not neighbors[0] == neighbors[1]:
                        ...
                        ref_atom = cg.get_closest_atom_of_element(cg.number_proton[ref_element], neighbor0,
                                                                  exclude=self)
