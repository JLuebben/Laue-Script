...
def generate_database(data, frequency_cutoff, clean=True, temperatures=None, path=None, apd_printer=None):
    ...
    if not apd_printer:
        ...
        printer = apd_printer
    errorlog = open('error.log', 'w')
    root = '/user/birger/NEUE_Datenbank/Modellverbindungen'
    #===========================================================================
                ...
                for freq in frequency_data:
                    if len(freq) > 3:
                        if atom_name == atom_names_list[0]:
                            data[compound_name].freq.append([freq[0], freq[1]])
                        num = len(data[compound_name].atoms) - 1
                        data[compound_name].atoms[-1].add_disps(freq[0],
                                                                freq[4 + num * 3:7 + num * 3])
...
def generate_micro_database(data,
    ...
    printer = pr.apd_printer(indent, __name__)
    ...
    for line in filepointer.readlines():
        if switch and 'Rotational constants' in line:
            ...
            switch = True
            matrix_buffer = Log_Buffer(clustersize, data, path, frequency_cutoff, printer)
...
class Log_Buffer(list):
    ...
    def __init__(self, clustersize, data, path, frequency_cutoff, printer):
        self.data = data
        ...
        for i, atom_name in enumerate(atom_names_list):
            ...
            for j, freq in enumerate(frequency_data[freq_num * -1:]):
                if len(freq) > 3:
                    ...
                    if atom_name == atom_names_list[0]:
                        if not freq[0] > 99999:
                            self.data['micro'].freq.append([freq[0], freq[1]])
                    num = len(self.data['micro'].atoms) - 1
                    if not freq[0] > 99999:
                        self.data['micro'].atoms[-1].add_disps(freq[0],
                                                               freq[4 + num * 3:7 + num * 3])
