...
def get_invariom_names(names,
    ...
    for l in xrange(len(thresholds)):
        ...
        for i, j, k in generator.harvest(l):
            # ===================================================================
            # try:
    ...
    for returnlist in returnlists:
        # =======================================================================
        # for name,value in returnlist[0].items():
        # print name,value
        #=======================================================================
        yield returnlist
...
class InvariomGenerator(object):
    ...
    def harvest(self, i):
        ...
        for key, value in self.invariom_names.items():
            yield key, value[i], self.orientations[key][i]
...
class Bond(object):
    ...
    def __init__(self, atom1, atom2, controller):
        ...
        if self.length < covalence_radius[atom1.get_element()] + covalence_radius[atom2.get_element()] + .1:
            ...
            # print self.length
            # print self.xi
            # print self.order,self.controller.get_tdt()
            # ===================================================================
...
# """
# A class representing an algorithm for finding n-membered
#     rings in molecules.
#     """
#
#     def __init__(self, angles, anglehashes):
#         """
#         Sets up the algorithm by given a list instances of the
#         Angle class as an argument.
#         """
#         self.points = []
#         self.angles = angles.values()
#         self.angle_dict = angles
#         self.hashes = anglehashes
#         for angle in self.angles:
#             self.points.append(Point(angle.get_center(), angle.get_id()))
#         self.search()
#
#     def search(self):
#         """
#         Searching for rings by analysing the properties of the
#         Angle class instances.
#         """
#         self.ring_centers = []
#         self.center_points = []
...
#                     if point1.check_position(point2):
#                         counter += 1
#             if counter > 3:
#                 string = '{1:.1f}:{2:.1f}:{3:.1f}:{0:.1f}'.format(1, *point1.get_best_center())
#                 if not string in self.ring_centers:
#                     self.ring_centers.append(string)
#                     self.center_points.append(Point(point1.get_best_center(), string))
#         self.verify()
#         self.find3()
#
#     def verify(self):
#         """
#         Verifies if a ring is a true ring by crossreferencing
#         the found center points with the Angle class instances'
#         properties.
#         """
#         self.accepted = []
...
#                 if center.verify(point):
#                     hits.append(point.get_id())
#             hits = sorted(hits)
#             string = ''
#             for hit in hits:
#                 string += '{}:'.format(hit)
#             string = string[:-1]
#             if not string in self.accepted:
#                 self.accepted.append(string)
#         self.rings = []
#         for string in self.accepted:
#             atoms = self.is_connected(string)
#             if atoms:
#                 if self.planar(string):
#                     self.rings.append(atoms)
#
#     def planar(self, strings):
#         """
#         Checks if a series of angle normals are parallel.
#         If they are, the atoms defining these angles
#         are co-planar.
#         """
#         check = 1
#         angle_ids = strings.split(':')
#         lengths = len(angle_ids)
#         for i, angle_id in enumerate(angle_ids):
#             check *= abs(np.dot(self.angle_dict[angle_id].get_normal(),
#                                 self.angle_dict[angle_ids[(i + 1) % lengths]].get_normal()))
#         if check > 0.98:
#             return True
#         else:
#             return False
#
#     def is_connected(self, string):
#         """
#         Conducts a final verification step by testing
#         whether all potential ring atoms are bound to
#         two other potential ring atoms.
#         """
#         angles = string.split(':')
#         atoms = []
#         for angle in angles:
...
#         for angle in self.angles:
#             h = angle.hash
#             if self.hashes.count(h) == 6:
#                 if not h in blacklist:
#                     self.rings.append(angle.get_id().split('*'))
#                     #===========================================================
#                     # self.rings.append()
#                     #===========================================================
#                     blacklist.append(h)
...
#     def harvest(self):
#         """
#         Returns the found rings by returning a list where
#         each element is representing a ring. Each ring
#         representation is a list of atom names that are
#         part of that ring.
#         """
#         return self.rings
#
...
#     The class is used by the RingFinder and encapsulates
#     several usefull methods and buffering information.
#     """
#     threshold = 0.33
#
#     def __init__(self, position, ID=None):
#         """
#         Initializes the Point instance by setting its
#         position and an optional ID.
#         """
#         self.id = ID
#         self.pos = position
#         self.checked = False
#         self.hits = [np.array(position)]
#
#     def get_best_center(self):
#         """
#         Computes and returns the center of a cluster
#         of Point instances.
#         """
#         self.best_center = np.array([0, 0, 0])
#         for pos in self.hits:
#             self.best_center = self.best_center + pos
#         self.best_center = self.best_center / len(self.hits)
...
#         """
#         Returns the Point's position in
#         cartesian coordinates.
#         """
#         return self.pos
#
...
#         """
#         Returns if the Point is closer to the instance point
#         than 0.6 Angstrom.
#         """
#         if self - point < 0.6:
#             return True
...
#         """
#         Returns True if point is closer to the Point instance
#         that Point.threshold.
#         """
#         if self - point < Point.threshold:
#             self.hits.append(point.get_position())
#             return True
#
#     def __sub__(self, point):
#         """
#         Returns a float representing the point instance's distance
#         to the Point instance itself.
#         """
#         return abs(norm(self.pos - point.get_position()))
#
    ...
    def build_invariom_name(self):
        ...
        if self.check_for_at_sorting:
            ...
            if name_part1.count('@') < name_part2.count('@'):
                name_list[self.check_indices[0] + 1], name_list[self.check_indices[1] + 1] = name_list[
                                                                                                 self.check_indices[
                                                                                                     1] + 1], name_list[
                                                                                                 self.check_indices[
                                                                                                     0] + 1]
...
class Chain(object):
    ...
    def get_chain_priority(self):
        ...
        for bond in self.branch.values():
            ...
            if bond.get_partner(self.root_atom).is_ring_atom():
